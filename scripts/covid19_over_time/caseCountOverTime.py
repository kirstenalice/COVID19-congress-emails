...
def get_case_by_district_over_time(state,start_date,end_date):
    ...
    for date in date_list:
        cbd = get_case_by_district(state,date)
        #print(cbd,state,date)
        cases = {k : v for k, v in sorted(cbd.items())}
        for k,v in cases.items():
            case_counts[k] += [v]

    return case_counts

def get_all_cases_by_state_over_time(start_date,end_date):
    if(not valid_date(start_date)):
    ...
    for state in states:
        ...
        row = [state]
        for date in dates:
            deaths_dict = get_state_case_data(state,date)
            death_sum = sum(list(deaths_dict.values()))
            row += [death_sum]
