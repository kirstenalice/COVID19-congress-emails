...
# returns True if date is valid calendar date, ex: 6/45/2020 -> False, 4/12/2020 -> True
#   and if the date falls in the range 1/22/2020 - 6/23/2020 (or whatever the most updated date in time_......csv)
def valid_date(date):
    try:
        parse(date)
    except ValueError:
    ...
    if(date_split[0] > 6 or date_split[0] < 1 or (date_split[0] == 6 and date_split[1] > 23) or (date_split[0] == 1 and date_split[1] < 22) or date_split[2] != 2020):
        print("invalid date: ",date)
        return False
    return True

# returns list of dates between start_date and end_date as strings in MM/DD/YYYY form
...
# county must be full name
# state must be full name, not abbrev ex. "New Jersey", not "NJ"
# start_date must be no earlier than 1/22/2020, following format MM/DD/YYYY
# end_date must be no later than 6/23/2020, following format MM/DD/YYYY (inclusive) (most recent date in time_series_covid19_deaths_US.csv)
...
# date must be no earlier than 1/22/2020 and no later than 6/23/2020 (or the most recent date w data)
...
# state must be full name, not abbrev ex. "New Jersey", not "NJ"
# date must be no earlier than 1/22/2020 and no later than the most recent day on time.....csv (MM/DD/YYYY)
...
if __name__ == '__main__':
    #get_all_deaths_by_district_over_time("1/22/2020","6/23/2020")

    #print(get_death_by_district_over_time("Louisiana","4/20/2020","4/26/2020"))

    #print(get_state_death_data("Alaska","4/20/2020"))
    ...
    for state in states:
        ...
        state_deaths_by_district = {k : v for k, v in sorted(get_death_by_district(state,"6/23/2020").items())}
        total = 0
